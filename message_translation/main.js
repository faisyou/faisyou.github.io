 ...
 */
let onMessage = (data) => {
    const _prefix=`[RTT:Main] [${currentConversationId}:onMessage] `;
    ...
    if(messageType === 'chat'){
        switch(data.metadata.type){
            ...
            case 'message':
                ...
                translate.translateText(message, genesysCloudLanguage, function(translatedData) {
                    ...
                    view.addChatMessageWithOriginal(name,translatedData.translated_text,message,purpose);
        ...
        if (data.topicName.toLowerCase() === 'channel.metadata') {
            ...
            console.debug(`${_prefix}Processing nParticipants:${data.eventBody.participants.length}`)
            data.eventBody.participants.forEach((participant) => {
                if(!participant.endTime) {
                    try {
                        ...
                        console.log(`Error processing Participant:${JSON.stringify(participant)}`);
                    }
                }
            });
            console.debug(`${_prefix}:nMessages:${messages.length}|Processing Batch|Most Recent Published Message:${mostRecentMessageTime}`);
            ...
            console.debug(`${_prefix}MostRecentMessageTime:${mostRecentMessageTime}:MessageId:${messageId}:${purpose}:Publish:${publish}`);

            name = (purpose === 'customer') ? customerName : agentAlias
            
            if (publish) {
                console.debug(`${_prefix}FetchingMessage:${messageId}}`);
                conversationsApi.getConversationsMessageMessage(data.eventBody.id, messageId)
                .then((messageDetail => {
                    // Make sure message is published only once
                    //console.debug(`${_prefix}:MessageDetail:${JSON.stringify(messageDetail)}`);
                    if(!messageIds.includes(messageId)){
                        // Ignore messages without text (e.g. Presence/Disconect Event)
                        
                        if (null != messageDetail.textBody) {
                            messageIds.push(messageId);
                            console.debug(`${_prefix}${messageId}|Sending for Translation:${messageDetail.textBody}`)

                            // Wait for translate to finish before calling addChatMessage
                            translate.translateText(messageDetail.textBody, genesysCloudLanguage, function(translatedData) {
                                console.debug(`${_prefix}Translation:${translatedData.translated_text}`)
                                //view.addChatMessage(name, translatedData.translated_text, purpose);
                                view.addChatMessageWithOriginal(name,translatedData.translated_text,messageDetail.textBody,purpose);
                                translationData = translatedData;
                            });
                        } else {
                            console.warn(`${_prefix}:${messageId} Not Published because textBody is null|MessageDetail:${JSON.stringify(messageDetail)}`);
                        }
                    }else {
                        console.log(`${_prefix}:${messageId} Not Published because its already published as its present in ${JSON.stringify(messageIds)}`);
                    }                   
                }));          
            }else {
                console.log(`${_prefix}${messageId} Not Published because Publish is false because mesage time is less then mostRecentPublishTime.`);
                console.log(`${_prefix}|MostRecentPublishedTime:${mostRecentMessageTime}|MessageDetail:${JSON.stringify(messageDetail)}`);
            }
        }
    }    
};
 ...
 */
function sendChat(){
    const _prefix=`[RTT:Main] [${currentConversationId}:sendChat] `
    ...
    console.debug(`${_prefix}Sending Agent Message:${message} for translation in language:${sourceLang}`);
    translate.translateText(message, sourceLang, function(translatedData) {
        ...
        console.debug(`${_prefix} sending translated text:${translatedData.translated_text} in the Conversation`);
        sendMessage(translatedData.translated_text,message, currentConversationId, communicationId);
    });
 ...
 */
function sendMessage(message,originalMessage, conversationId, communicationId){
    const _prefix=`[RTT:main][${conversationId}:sendMessage]`;
    ...
    if(messageType === 'chat') {
        ...
        conversationsApi.postConversationsMessageCommunicationMessages(
            ...
            console.debug(`${_prefix} Message sent.MessageId:${messageId}|Status:${status}|Direction:${direction}`);
            view.addChatMessageWithOriginal(agentAlias,originalMessage,message,'agent');
            messageIds.push(messageId);
        }).error((e)=>{
            console.warn(`${_prefix} Unable to Publish Message|CommunicationId:${communicationId}|Message:${message}`);
        });
    }    
}
...
function setupChatChannel(conversationId){
    const _prefix=`[RTT:Main] [${conversationId} setupChatChannel]`;
...
console.log(`[${currentConversationId}:InitialSetup] Starting Translation Widget.ConversationId:${currentConversationId}|genesysCloudLanguage:${genesysCloudLanguage}`);
...
.then(data => {
    ...
    const _prefix=`[RTT:Main] [${currentConversationId} AgentLang:${genesysCloudLanguage} ]`;
    ...
    const _prefix=`[RTT:Main] [${currentConversationId} Agent:${agentAlias} ]`;
    console.log(`${_prefix}`)
    ...
    const _prefix=`[RTT:Main] [${currentConversationId} MessageType:${messageType} ] `;
    ...
    console.log(`${_prefix} fetching chat transcript prior to agent`);
