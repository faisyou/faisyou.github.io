...
let subscriptionMap = {
    'channel.metadata': () => {
        console.log('RTT:Notification:heartbeat.');
    }
};
...
export default {
    ...
    createChannel(){
        ...
        .then(data => {
            console.log(`RTT:Notification: Created Notifications ChannelsData:${JSON.stringify(data)}`);
            
            channel = data;
            ws = new WebSocket(channel.connectUri);
            ws.onmessage = onSocketMessage;
            ws.onclose = function(closingEvent) {
                console.log(`RTT:Notification:Web-socket Closed.Channel:${JSON.stringify(channel)}|CloseEvent:${JSON.stringify(closingEvent)}`);
            };
            ws.onerror =function(errorEvent) {
                console.log(`RTT:Notification:Web-socket Error.Channel:${JSON.stringify(channel)}|ErrorEvent:${JSON.stringify(errorEvent)}`);
            };
        });
    },
    ...
    addSubscription(topic, callback){
        let body = [{'id': topic}]
        console.debug(`RTT:Notification:Adding Subscription Topic:${topic}`);
        return notificationsApi.postNotificationsChannelSubscriptions(
                channel.id, body)
        .then((data) => {
            subscriptionMap[topic] = callback;
            console.log(`RTT:Notification:Added subscription to ${topic}`);
        });
    }
}